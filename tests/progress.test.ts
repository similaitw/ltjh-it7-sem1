...
describe('課程內容', () => {
  ...
  it.each(lessons)('$id 有摘要、觀念、課堂活動、完成條件與 checklist', (lesson) => {
  ...
  it('涵蓋 Scratch 與 Google 園遊會的必需內容', () => {
    const scratch = JSON.stringify(chapters[1]);
    for (const topic of ['流程圖', '循序', '選擇', '重複', '座標', '畫筆']) expect(scratch).toContain(topic);
    const project = JSON.stringify(chapters[2]);
    for (const topic of ['Google 搜尋', 'Google 文件', 'Google 試算表', 'Google 簡報', '園遊會']) expect(project).toContain(topic);
  });
});
